...
class StableDiffusionEngine:
    def __init__(
        ...
        self.text_encoder_torch = CLIPTextModel.from_pretrained(
                            pretrained_model_name_or_path
                            )
        # 加载学习的风格文件
        if not file_embeding is None or len(file_embeding)>0:
            for file in file_embeding:
                str_token = load_learned_embed_in_clip(file,self.text_encoder_torch,self.tokenizer)
                print(str_token)
        ...
        self._vae_encoder = self.core.read_model(
            ...
            os.path.join(model,"vae_encoder.bin")
        )
    ...
    def __call__(
        ...
        text_embeddings = self.text_encoder_torch(text_input.input_ids.to('cpu'))[0]
        ...
        if guidance_scale > 1.0:
            ...
            uncond_embeddings = self.text_encoder_torch(tokens_uncond.input_ids.to('cpu'))[0]
            uncond_embeddings = uncond_embeddings.detach().numpy()
